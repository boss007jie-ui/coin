...
function createCexBackgroundMonitor({
  ...
  function formatPaperTradingAlert(result) {
    ...
    const openedLines = opened.slice(0, 5).map((trade) => (
      `开仓 ${trade.symbol} ${trade.side === "short" ? "空" : "多"} ${trade.leverage}x / 保证金 ${formatUsdt(trade.marginUsdt)} / SL ${formatPrice(trade.stopLossPrice)} / TP ${formatPrice(trade.takeProfitPrice)}`
    ));
    const closedLines = closed.slice(0, 5).map((trade) => (
      `平仓 ${trade.symbol} ${trade.exitReason} / PnL ${formatSignedUsdt(trade.pnlUsdt)}`
    ));
  ...
  function formatPaperPeriodSummary({ title, periodLabel, trades, account, filterTrade }) {
