...
function createCexBackgroundMonitor({
  ...
  function formatPaperTradingAlert(result) {
    ...
    const openedLines = opened.slice(0, 5).map((trade) => (
      `开仓 ${formatExperimentGroup(trade)} ${trade.symbol} ${trade.side === "short" ? "空" : "多"} ${trade.leverage}x / ${marginModeLabel(trade.marginMode)} / 保证金 ${formatUsdt(trade.marginUsdt)} / 移动SL ${formatPrice(trade.stopLossPrice)} / TP ${formatPrice(trade.takeProfitPrice)}`
    ));
