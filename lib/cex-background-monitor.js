...
const PAPER_DAILY_LOSS_LIMIT_PCT = 5;
const DEFAULT_PAPER_STRATEGY_VERSION = "2026-06-23-multi-evidence-reset-v1";
...
function createCexBackgroundMonitor({
  ...
  paperStrategyVersion = DEFAULT_PAPER_STRATEGY_VERSION,
  logger = console
}) {
  ...
  async function maybeRunPaperTrading(tokens, currentTime) {
    ...
    try {
      let ledger = await loadPaperTrades();
      ...
      const nextPaperState = paperState && typeof paperState === "object" && !Array.isArray(paperState)
        ? { ...paperState }
        : {};
      const reset = await maybeResetPaperTradingForStrategyVersion(ledger, nextPaperState, currentTime);
      if (reset) {
        ledger = [];
        await sendNotification(formatPaperStrategyResetAlert(reset, currentTime));
      }
      ...
      const result = await runPaperTradingCycle({
        ...
        strategyProfile,
        dailyLossGuard: {
          dateKey: toShanghaiDateKey(currentTime),
          maxLossPct: PAPER_DAILY_LOSS_LIMIT_PCT,
          maxLossUsdt: DEFAULT_PAPER_ACCOUNT.initialEquityUsdt * (PAPER_DAILY_LOSS_LIMIT_PCT / 100)
        }
      });
      const paperFeedback = result.feedback || buildPaperFeedbackSummary(result.trades, { now: currentTime });
      ...
      await maybeSendDailyLossEmergencyReview(result, paperFeedback, nextPaperState, currentTime);
      ...
      state.lastPaperTrading = {
        ...
        feedback: summarizePaperFeedbackForStatus(paperFeedback),
        dailyLossGuard: result.dailyLossGuard || null,
        strategyVersion: nextPaperState.strategyVersion || paperStrategyVersion,
        lastStrategyResetAt: nextPaperState.lastStrategyResetAt || null,
        lastStrategyResetPreviousVersion: nextPaperState.lastStrategyResetPreviousVersion || null
      };
  ...
  async function maybeResetPaperTradingForStrategyVersion(ledger, paperState, currentTime) {
    const currentVersion = normalizeStrategyVersion(paperStrategyVersion);
    if (!currentVersion) return null;
    const previousVersion = normalizeStrategyVersion(paperState.strategyVersion);
    if (!previousVersion) {
      paperState.strategyVersion = currentVersion;
      return null;
    }
    if (previousVersion === currentVersion) return null;

    const trades = Array.isArray(ledger) ? ledger : [];
    let archive = null;
    if (trades.length && typeof archivePaperTrades === "function") {
      archive = await archivePaperTrades(trades, {
        reason: "strategy-version-change",
        previousStrategyVersion: previousVersion || "unversioned",
        nextStrategyVersion: currentVersion,
        resetAt: currentTime.toISOString(),
        initialEquityUsdt: DEFAULT_PAPER_ACCOUNT.initialEquityUsdt
      });
    }

    const priorResetCount = Number(paperState.strategyResetCount) || 0;
    paperState.strategyVersion = currentVersion;
    paperState.strategyProfile = "standard";
    paperState.lastStrategyResetAt = currentTime.toISOString();
    paperState.lastStrategyResetPreviousVersion = previousVersion || "unversioned";
    paperState.lastStrategyResetReason = "strategy-version-change";
    paperState.strategyResetCount = priorResetCount + 1;
    delete paperState.lastDailyLossStopDateKey;
    delete paperState.lastDailyLossStopAt;
    delete paperState.lastDailyLossStopPnlUsdt;
    delete paperState.lastCapitalStopDateKey;
    delete paperState.lastCapitalStopAt;
    delete paperState.lastCapitalStopEquityUsdt;

    return {
      reason: "strategy-version-change",
      previousStrategyVersion: previousVersion || "unversioned",
      nextStrategyVersion: currentVersion,
      archivedCount: trades.length,
      archive,
      initialEquityUsdt: DEFAULT_PAPER_ACCOUNT.initialEquityUsdt
    };
  }
  ...
  async function maybeSendDailyLossEmergencyReview(result, feedback, paperState, currentTime) {
    const guard = result.dailyLossGuard || {};
    if (!guard.halted) return;
    const localDateKey = guard.dateKey || toShanghaiDateKey(currentTime);
    if (paperState.lastDailyLossStopDateKey === localDateKey) return;

    const message = formatDailyLossEmergencyReview(result, feedback, currentTime);
    const sendResult = await sendNotification(message);
    if (sendResult.ok === false) return;

    paperState.strategyProfile = "defensive-v1";
    paperState.lastDailyLossStopDateKey = localDateKey;
    paperState.lastDailyLossStopAt = currentTime.toISOString();
    paperState.lastDailyLossStopPnlUsdt = guard.pnlUsdt;
    paperState.dailyLossStopCount = (Number(paperState.dailyLossStopCount) || 0) + 1;
  }
  ...
  function formatPaperTradingAlert(result) {
    ...
    const openedLines = opened.slice(0, 5).map((trade) => (
      `开仓 ${formatExperimentGroup(trade)} ${trade.symbol} ${trade.side === "short" ? "空" : "多"} ${trade.leverage}x / ${marginModeLabel(trade.marginMode)} / 保证金 ${formatUsdt(trade.marginUsdt)} / 移动SL ${formatPrice(trade.stopLossPrice)} / TP ${formatPrice(trade.takeProfitPrice)} / ${formatEntryEvidenceSummary(trade.entryEvidence)}`
    ));
  ...
  function formatPaperStrategyResetAlert(reset, currentTime) {
    return [
      "[CEX 模拟交易] 策略版本变更，模拟账户重启",
      `触发时间: ${toShanghaiDateTimeLabel(currentTime)}`,
      `版本: ${reset.previousStrategyVersion} -> ${reset.nextStrategyVersion}`,
      `动作: 旧账本归档 ${reset.archivedCount || 0} 笔，新本金 ${formatUsdt(reset.initialEquityUsdt)} 重新开仓`,
      reset.archive?.filePath ? `归档: ${reset.archive.filePath}` : "归档: 无旧交易"
    ].join("\n");
  }
  ...
  function formatDailyLossEmergencyReview(result, feedback, currentTime) {
    const guard = result.dailyLossGuard || {};
    const worstSetups = (feedback?.needsReview || feedback?.worstSetups || []).slice(0, 3);
    const worstSetupLine = worstSetups.length
      ? worstSetups.map(formatPaperSetupFeedback).join(" / ")
      : "--";

    return [
      "[CEX 模拟交易] 单日亏损超过 5%，紧急复盘",
      `触发时间: ${toShanghaiDateTimeLabel(currentTime)}`,
      `当日已实现PnL: ${formatSignedUsdt(guard.pnlUsdt)} / 阈值: -${formatUsdt(guard.maxLossUsdt)} / 亏损幅度: ${formatPctValue(guard.lossPct)}`,
      `策略动作: 当天暂停新开仓，切换 defensive-v1；已有持仓继续按移动止损和信号退出管理`,
      `本轮: 开仓 ${result.openedCount || 0} / 平仓 ${result.closedCount || 0} / 跳过 ${result.skippedCount || 0}`,
      `需复盘setup: ${worstSetupLine}`
    ].join("\n");
  }
  ...
  function formatPaperPeriodSummary({ title, periodLabel, trades, account, accountsByGroup, filterTrade }) {
    ...
    const feedbackLines = formatPaperFeedbackSummaryLines(buildPaperFeedbackSummary(closed));
  ...
  function formatPaperFeedbackSummaryLines(feedback) {
    if (!feedback || !feedback.closedCount) return [];
    const reviewSetups = (feedback.needsReview || []).slice(0, 3);
    const reviewKeys = new Set(reviewSetups.map((setup) => setup.key));
    const worstSetups = (feedback.worstSetups || [])
      .filter((setup) => !reviewKeys.has(setup.key))
      .slice(0, 2);
    const lines = [];
    if (reviewSetups.length) {
      lines.push(`需复盘: ${reviewSetups.map(formatPaperSetupFeedback).join(" / ")}`);
    }
    if (worstSetups.length) {
      lines.push(`最差组合: ${worstSetups.map(formatPaperSetupFeedback).join(" / ")}`);
    }
    return lines;
  }

  function formatPaperSetupFeedback(setup) {
    return `${setup.experimentGroup}/${setup.experimentGroupLabel} ${setup.actionSetup}/${setup.reviewLabel}/${setup.phase} 样本 ${setup.sampleSize} / 胜率 ${formatPctValue(setup.winRatePct)} / PnL ${formatSignedUsdt(setup.totalPnlUsdt)} / 连亏 ${setup.maxLossStreak}`;
  }
...
function formatEntryEvidenceSummary(entryEvidence) {
  const evidence = entryEvidence && typeof entryEvidence === "object" ? entryEvidence : {};
  const angleCount = Number.isFinite(Number(evidence.angleCount)) ? Number(evidence.angleCount) : 0;
  const minRequired = Number.isFinite(Number(evidence.minRequiredConfirmations)) ? Number(evidence.minRequiredConfirmations) : 0;
  const confirmations = Array.isArray(evidence.confirmations) ? evidence.confirmations : [];
  const labels = confirmations
    .slice(0, 3)
    .map((item) => item.label)
    .filter(Boolean);
  return `证据 ${angleCount}/${minRequired}: ${labels.length ? labels.join(" / ") : "--"}`;
}

function marginModeLabel(value) {
  return value === "cross" ? "全仓" : "逐仓";
}
...
function summarizePaperFeedbackForStatus(feedback) {
  if (!feedback) return null;
  return {
    generatedAt: feedback.generatedAt,
    closedCount: feedback.closedCount,
    setupCount: feedback.setupCount,
    needsReviewCount: feedback.needsReviewCount,
    needsReview: (feedback.needsReview || []).slice(0, 5),
    worstSetups: (feedback.worstSetups || []).slice(0, 5)
  };
}
